...
class PainPointContext(BaseModel):
    ...
    current_tools: Optional[List[str]] = Field(default_factory=list, description="Current tools")
...
class CurrentImpactMetrics(BaseModel):
    ...
    customer_satisfaction: Optional[str] = Field(None, description="Customer satisfaction score")
...
class CurrentImpact(BaseModel):
    ...
    metrics: Optional[CurrentImpactMetrics] = Field(default_factory=CurrentImpactMetrics)
...
class UserPreferences(BaseModel):
    """User preferences"""
    solution_types: Optional[List[str]] = Field(default_factory=list, description="Preferred solution types")
    implementation_timeline: Optional[str] = Field(None, description="Implementation timeline")
    integration_requirements: Optional[str] = Field(None, description="Integration requirements")
...
class ExpectedOutcomes(BaseModel):
    ...
    short_term: List[str] = Field(default_factory=list, description="Short-term results")
...
class Solution(BaseModel):
    ...
    filum_features: List[FilumFeature] = Field(..., description="Related Filum.ai features")
    ...
    expected_outcomes: Optional[ExpectedOutcomes] = Field(default_factory=ExpectedOutcomes)
    ...
    resource_requirements: Optional[ResourceRequirements] = Field(default_factory=ResourceRequirements)
    success_metrics: List[str] = Field(default_factory=list, description="Success metrics")
    related_case_studies: Optional[List[str]] = Field(default_factory=list, description="Related case studies")
...
class AlternativeApproach(BaseModel):
    ...
    pros_cons: Dict[str, List[str]] = Field(default_factory=dict, description="Pros and cons")


class NextSteps(BaseModel):
    """Next steps"""
    immediate_actions: List[str] = Field(default_factory=list, description="Immediate actions")
    consultation_needed: Optional[bool] = Field(None, description="Need additional consultation")
    demo_requests: List[str] = Field(default_factory=list, description="Available demo requests")


class PainPointAnalysis(BaseModel):
    """Results analysis pain point"""
    pain_point_summary: str = Field(..., description="Problem summary")
    key_challenges: List[str] = Field(default_factory=list, description="Key challenges")
...
class AgentOutput(BaseModel):
    ...
    alternative_approaches: Optional[List[AlternativeApproach]] = Field(default_factory=list)
...
class PainPointAddressed(BaseModel):
    ...
    severity_levels: List[UrgencyLevel] = Field(default_factory=list, description="Severity levels")
    business_contexts: List[str] = Field(default_factory=list, description="Business contexts")
...
class IntegrationOptions(BaseModel):
    ...
    third_party_tools: List[str] = Field(default_factory=list, description="Third-party tools")
...
class FeatureBenefits(BaseModel):
    """Feature benefits"""
    quantitative: List[str] = Field(default_factory=list, description="Quantitative benefits")
    qualitative: List[str] = Field(default_factory=list, description="Qualitative benefits")
...
class KnowledgeBaseFeature(BaseModel):
    ...
    integration_options: Optional[IntegrationOptions] = Field(default_factory=IntegrationOptions)
    implementation: ImplementationDetails = Field(..., description="Implementation details")
...
class PainPointCategory(BaseModel):
    ...
    indicators: List[str] = Field(default_factory=list, description="Recognition indicators")
...
class BusinessContexts(BaseModel):
    ...
    company_sizes: List[CompanySize] = Field(default_factory=list, description="Company sizes")
    business_models: List[str] = Field(default_factory=list, description="Business models")


class KnowledgeBase(BaseModel):
    """Overall knowledge base"""
    features: List[KnowledgeBaseFeature] = Field(default_factory=list)
    pain_point_taxonomy: Optional[PainPointTaxonomy] = Field(default_factory=PainPointTaxonomy)
    business_contexts: Optional[BusinessContexts] = Field(default_factory=BusinessContexts)
