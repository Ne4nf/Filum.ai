...
def analyze(
    input_file: Optional[str] = typer.Option(
        ...
        "--input", 
        "-i", 
        help="Path to JSON file containing pain point input"
    ),
    ...
    max_solutions: int = typer.Option(
        ...
        "--max-solutions",
        "-m",
        help="Maximum number of solutions to recommend"
    ),
    knowledge_base: Optional[str] = typer.Option(
        None,
        "--kb",
        help="Path to custom knowledge base file"
    ),
    interactive: bool = typer.Option(
        False,
        "--interactive",
        "-I",
        help="Interactive mode to input pain point"
    )
):
    ...
    try:
        ...
        if interactive:
            ...
            console.print("❌ Need to provide --input file or use --interactive mode", style="red")
            raise typer.Exit(1)
    ...
    try:
        ...
        for feature in features:
            table.add_row(
                ...
                feature["category"],
                feature["description"][:60] + "..." if len(feature["description"]) > 60 else feature["description"]
            )
...
def feature_detail(feature_id: str):
    ...
    try:
        ...
        console.print(f"🔍 Feature Details: {feature_detail['feature_name']}", style="bold blue")
        console.print()
        ...
        console.print(f"🔧 Complexity: {feature_detail['implementation']['complexity']}", style="yellow")
        console.print(f"⏱️ Setup Time: {feature_detail['implementation']['setup_time']}", style="yellow")
...
def get_interactive_input() -> dict:
    ...
    console.print("📝 Interactive Mode - Enter pain point information:", style="bold blue")
    console.print()
...
def save_output_file(result, file_path: str):
    ...
    try:
        ...
        if output_path.suffix.lower() == '.json':
            ...
            console.print("❌ Only .json and .md output formats are supported", style="red")
            return
...
def display_analysis_results(result):
    ...
    if result.recommended_solutions:
        ...
        for i, solution in enumerate(result.recommended_solutions, 1):
            console.print(f"🚀 Solution {i}: {solution.solution_name}", style="bold cyan")
...
def handle_demo_requests(solutions):
    ...
    for i, solution in enumerate(solutions, 1):
        ...
        if request_demo:
            console.print(f"✅ Demo request recorded for {solution.solution_name}", style="green")
