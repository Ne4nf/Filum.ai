...
class PainPointAgent:
    ...
    def analyze_and_recommend(
        self, 
        pain_point_input: Dict | PainPointInput,
        max_solutions: int = 3
    ) -> AgentOutput:
        """
    ...
    def _assess_impact(self, pain_point: PainPointInput) -> str:
        ...
        if context and context.urgency_level:
            ...
            impact_factors.append(urgency_map.get(urgency, 'Impact level needs further assessment'))
        ...
        if current_impact and current_impact.metrics:
            ...
            if metrics.customer_satisfaction and 'low' in str(metrics.customer_satisfaction).lower():
                impact_factors.append("Customer satisfaction is being negatively affected")
            if metrics.response_time and any(word in str(metrics.response_time).lower() 
                                           for word in ['slow', 'high']):
                impact_factors.append("Current response time does not meet requirements")
        
        # Company size impact
        if context and context.company_size:
            size = context.company_size.value
            if size in ['large', 'enterprise']:
                impact_factors.append("At large scale, impact may affect many customers")
    ...
    def _generate_next_steps(
        ...
        if pain_point.pain_point.context:
            ...
            if context.urgency_level and context.urgency_level.value == 'high':
                immediate_actions.insert(0, "Organize urgent meeting with leadership team")
        ...
        if solutions and len(solutions) > 0:
            avg_complexity = sum(1 if sol.complexity_level.value == 'low' else 
                               2 if sol.complexity_level.value == 'medium' else 3 
    ...
    def export_analysis_report(
        ...
        if format.lower() == "json":
            ...
            raise ValueError("Format not supported. Only 'json' and 'markdown' are supported")
